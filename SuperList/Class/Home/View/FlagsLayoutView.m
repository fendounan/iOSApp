...
@implementation FlagsLayoutView
...
-(void)setTagWithTagArray:(NSArray*)arr{
    ...
    for (NSString *str in arr) {
        ...
        if (_BigBGColor) {
            tag.layer.borderColor = _BigBGColor.CGColor;
            tag.layer.borderWidth = 0.5;
        }
        tag.backgroundColor = [UIColor clearColor];
        tag.textAlignment = NSTextAlignmentCenter;
        tag.textColor = [UIColor hexStringToColor:@"#666666"];
        tag.font = [UIFont systemFontOfSize:12 weight:UIFontWeightThin];
        tag.fixFontSize = YES;
        tag.adjustsFontSizeToFitWidth = NO;
        tag.text = str;
        //fix 哈哈
        CGSize textStrSize = [str sizeWithAttributes:@{NSFontAttributeName:tag.font}];
