...
{
    __weak HKNewsBannerView *wself = self;

    [UIView animateWithDuration:0.25 animations:^{
        if (wself.scrollDirection == HKNewsBannerViewScrollDirectionLandscape) {
            wself.scrollView.contentOffset = CGPointMake(1.99 * wself.scrollView.frame.size.width, 0);
        }else{
            wself.scrollView.contentOffset = CGPointMake(0, 1.99 * wself.scrollView.frame.size.height);
        }
    } completion:^(BOOL finished) {
        if (finished) {
            wself.currentPage = [self getPageIndex:(self.currentPage + 1)];
            [wself refreshScrollView];
            [NSObject cancelPreviousPerformRequestsWithTarget:wself selector:@selector(rollingScrollAction) object:nil];
            [wself performSelector:@selector(rollingScrollAction) withObject:nil afterDelay:wself.rollingDelayTime inModes:@[NSRunLoopCommonModes]];
        }
    }];
}
